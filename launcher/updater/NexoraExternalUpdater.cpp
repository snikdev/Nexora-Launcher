...
#include "NexoraExternalUpdater.h"
...
class NexoraExternalUpdater::Private {
   public:
    QDir appDir;
    QDir dataDir;
    QTimer updateTimer;
...
NexoraExternalUpdater::NexoraExternalUpdater(QWidget* parent, const QString& appDir, const QString& dataDir)
    : priv(new NexoraExternalUpdater::Private())
{
    priv->appDir = QDir(appDir);
    priv->dataDir = QDir(dataDir);
    auto settings_file = priv->dataDir.absoluteFilePath("nexoralauncher_update.cfg");
...
NexoraExternalUpdater::~NexoraExternalUpdater()
{
...
void NexoraExternalUpdater::checkForUpdates()
{
    checkForUpdates(true);
}

void NexoraExternalUpdater::checkForUpdates(bool triggeredByUser)
{
...
bool NexoraExternalUpdater::getAutomaticallyChecksForUpdates()
{
    return priv->autoCheck;
}

double NexoraExternalUpdater::getUpdateCheckInterval()
{
    return priv->updateInterval;
}

bool NexoraExternalUpdater::getBetaAllowed()
{
    return priv->allowBeta;
}

void NexoraExternalUpdater::setAutomaticallyChecksForUpdates(bool check)
{
...
void NexoraExternalUpdater::setUpdateCheckInterval(double seconds)
{
...
void NexoraExternalUpdater::setBetaAllowed(bool allowed)
{
...
void NexoraExternalUpdater::resetAutoCheckTimer()
{
...
void NexoraExternalUpdater::connectTimer()
{
    connect(&priv->updateTimer, &QTimer::timeout, this, &NexoraExternalUpdater::autoCheckTimerFired);
}

void NexoraExternalUpdater::disconnectTimer()
{
    disconnect(&priv->updateTimer, &QTimer::timeout, this, &NexoraExternalUpdater::autoCheckTimerFired);
}

void NexoraExternalUpdater::autoCheckTimerFired()
{
    qDebug() << "Auto update Timer fired";
    checkForUpdates(false);
}

void NexoraExternalUpdater::offerUpdate(const QString& version_name, const QString& version_tag, const QString& release_notes)
{
...
void NexoraExternalUpdater::performUpdate(const QString& version_tag)
{
    QProcess proc;
    auto exe_name = QStringLiteral("%1_updater").arg(BuildConfig.LAUNCHER_APP_BINARY_NAME);
#if defined Q_OS_WIN32
