...
#include "updater/nexoraupdater/UpdaterDialogs.h"
...
void appDebugOutput(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    NexoraUpdaterApp* app = static_cast<NexoraUpdaterApp*>(QCoreApplication::instance());
...
NexoraUpdaterApp::NexoraUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for Nexora Launcher"));
    ...
    if (!dirParam.isEmpty()) {
        // the dir param. it makes nexora launcher data path point to whatever the user specified
        // on command line
    ...
    m_updateLogPath = FS::PathCombine(m_dataPath, "logs", "nexora_launcher_update.log");
    ...
    QString nexora_executable = FS::PathCombine(binPath, BuildConfig.LAUNCHER_APP_BINARY_NAME);
#if defined Q_OS_WIN32
    nexora_executable.append(".exe");
#endif

    if (!QFileInfo(nexora_executable).isFile()) {
        showFatalErrorMessage(tr("Unsupported Installation"), tr("The updater can not find the main executable."));
    }

    m_nexoraExecutable = nexora_executable;

    auto nexora_update_url = parser.value("update-url");
    if (nexora_update_url.isEmpty())
        nexora_update_url = BuildConfig.UPDATER_GITHUB_REPO;

    m_nexoraRepoUrl = QUrl::fromUserInput(nexora_update_url);
    ...
    auto version = parser.value("nexora-version");
    if (!version.isEmpty()) {
        if (version.contains('-')) {
        ...
        m_nexoraVersionMajor = version_parts.takeFirst().toInt();
        m_nexoraVersionMinor = version_parts.takeFirst().toInt();
        if (!version_parts.isEmpty())
            m_nexoraVersionPatch = version_parts.takeFirst().toInt();
        else
            m_nexoraVersionPatch = 0;
    }

    m_allowPreRelease = parser.isSet("pre-release");

    auto marker_file_path = QDir(m_rootPath).absoluteFilePath(".nexora_launcher_updater_unpack.marker");
    ...
    if (marker_file.exists()) {
        ...
        QMetaObject::invokeMethod(this, &NexoraUpdaterApp::loadReleaseList, Qt::QueuedConnection);
    }
}

NexoraUpdaterApp::~NexoraUpdaterApp()
{
...
void NexoraUpdaterApp::fail(const QString& reason)
{
...
void NexoraUpdaterApp::abort(const QString& reason)
{
    qCritical() << qPrintable(reason);
    m_status = Aborted;
    exit(2);
...
void NexoraUpdaterApp::showFatalErrorMessage(const QString& title, const QString& content)
{
    m_status = Failed;
...
void NexoraUpdaterApp::run()
{
    qDebug() << "found" << m_releases.length() << "releases on github";
    qDebug() << "loading exe at" << m_nexoraExecutable;
    ...
    if (!loadNexoraVersionFromExe(m_nexoraExecutable)) {
        m_nexoraVersion = BuildConfig.printableVersionString();
        m_nexoraVersionMajor = BuildConfig.VERSION_MAJOR;
        m_nexoraVersionMinor = BuildConfig.VERSION_MINOR;
        m_nexoraVersionPatch = BuildConfig.VERSION_PATCH;
        m_prsimVersionChannel = BuildConfig.VERSION_CHANNEL;
        m_nexoraGitCommit = BuildConfig.GIT_COMMIT;
    }
    m_status = Succeeded;

    qDebug() << "Executable reports as:" << m_nexoraBinaryName << "version:" << m_nexoraVersion;
    qDebug() << "Version major:" << m_nexoraVersionMajor;
    qDebug() << "Version minor:" << m_nexoraVersionMinor;
    qDebug() << "Version minor:" << m_nexoraVersionPatch;
    qDebug() << "Version channel:" << m_prsimVersionChannel;
    qDebug() << "Git Commit:" << m_nexoraGitCommit;
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of Nexora Launcher."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
...
void NexoraUpdaterApp::moveAndFinishUpdate(QDir target)
{
    ...
    if (error) {
        ...
        auto fail_marker = FS::PathCombine(m_dataPath, ".nexora_launcher_update.fail");
        FS::copy(m_updateLogPath, fail_marker).overwrite(true)();
    } else {
        logUpdate(tr("Update succeed."));
        auto success_marker = FS::PathCombine(m_dataPath, ".nexora_launcher_update.success");
        FS::copy(m_updateLogPath, success_marker).overwrite(true)();
    }
    auto update_lock_path = FS::PathCombine(m_dataPath, ".nexora_launcher_update.lock");
...
void NexoraUpdaterApp::printReleases()
{
...
QList<GitHubRelease> NexoraUpdaterApp::nonDraftReleases()
{
...
QList<GitHubRelease> NexoraUpdaterApp::newerReleases()
{
    QList<GitHubRelease> newer;
    for (auto rls : nonDraftReleases()) {
        if (rls.version > m_nexoraVersion)
            newer.append(rls);
    }
    return newer;
}

GitHubRelease NexoraUpdaterApp::selectRelease()
{
    ...
    SelectReleaseDialog dlg(Version(m_nexoraVersion), releases);
...
QList<GitHubReleaseAsset> NexoraUpdaterApp::validReleaseArtifacts(const GitHubRelease& release)
{
...
GitHubReleaseAsset NexoraUpdaterApp::selectAsset(const QList<GitHubReleaseAsset>& assets)
{
...
void NexoraUpdaterApp::performUpdate(const GitHubRelease& release)
{
...
QFileInfo NexoraUpdaterApp::downloadAsset(const GitHubReleaseAsset& asset)
{
...
bool NexoraUpdaterApp::callAppImageUpdate()
{
...
void NexoraUpdaterApp::clearUpdateLog()
{
    FS::deletePath(m_updateLogPath);
}

void NexoraUpdaterApp::logUpdate(const QString& msg)
{
...
void NexoraUpdaterApp::performInstall(QFileInfo file)
{
    ...
    if (update_lock.exists()) {
        ...
        auto infoMsg =
            tr("This installation has a update lock file present at: %1\n"
               ...
               "proceeding.\n"
               "Check the Nexora Launcher updater log at: \n"
               "%7\n"
    ...
    auto changelog_path = FS::PathCombine(m_dataPath, ".nexora_launcher_update.changelog");
    FS::write(changelog_path, m_install_release.body.toUtf8());

    logUpdate(tr("Updating from %1 to %2").arg(m_nexoraVersion).arg(m_install_release.tag_name));
    if (m_isPortable || file.fileName().endsWith(".zip") || file.fileName().endsWith(".tar.gz")) {
        write_lock_file(update_lock_path, QDateTime::currentDateTime(), m_nexoraVersion, m_install_release.tag_name, m_rootPath, m_dataPath);
...
void NexoraUpdaterApp::unpackAndInstall(QFileInfo archive)
{
    logUpdate(tr("Backing up install"));
    backupAppDir();

    if (auto loc = unpackArchive(archive)) {
        auto marker_file_path = loc.value().absoluteFilePath(".nexora_launcher_updater_unpack.marker");
...
void NexoraUpdaterApp::backupAppDir()
{
    auto manifest_path = FS::PathCombine(m_rootPath, "manifest.txt");
    QFileInfo manifest(manifest_path);
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "NexoraLauncher", "bin", "share", "lib" });
        } else {  // windows by process of elimination
            file_list.append({
                "jars",
                "nexoralauncher.exe",
                "nexoralauncher_filelink.exe",
                "nexoralauncher_updater.exe",
                "qtlogging.ini",
                "imageformats",
                "iconengines",
                "platforms",
    ...
    auto backup_dir =
        FS::PathCombine(app_dir.absolutePath(),
                        QStringLiteral("backup_") + QString(m_nexoraVersion).replace(s_replaceRegex, QString("_")) + "-" + m_nexoraGitCommit);
    FS::ensureFolderPathExists(backup_dir);
    auto backup_marker_path = FS::PathCombine(m_dataPath, ".nexora_launcher_update_backup_path.txt");
...
std::optional<QDir> NexoraUpdaterApp::unpackArchive(QFileInfo archive)
{
    auto temp_extract_path = FS::PathCombine(m_dataPath, "nexora_launcher_update_release");
...
bool NexoraUpdaterApp::loadNexoraVersionFromExe(const QString& exe_path)
{
    QProcess proc = QProcess();
    ...
    if (first_parts.length() < 2)
        return false;
    m_nexoraBinaryName = first_parts.takeFirst();
    auto version = first_parts.takeFirst().trimmed();
    m_nexoraVersion = version;
    if (version.contains('-')) {
        auto index = version.indexOf('-');
        m_prsimVersionChannel = version.mid(index + 1);
        version = version.left(index);
    ...
    if (!version_parts.isEmpty())
        ...
        m_nexoraVersionPatch = 0;
    m_nexoraGitCommit = lines.takeFirst().simplified();
    return true;
}

void NexoraUpdaterApp::loadReleaseList()
{
    auto github_repo = m_nexoraRepoUrl;
...
void NexoraUpdaterApp::downloadReleasePage(const QString& api_url, int page)
{
    int per_page = 30;
    ...
    connect(download.get(), &Net::Download::failed, this, &NexoraUpdaterApp::downloadError);
...
int NexoraUpdaterApp::parseReleasePage(const QByteArray* response)
{
...
GitHubRelease NexoraUpdaterApp::getLatestRelease()
{
...
bool NexoraUpdaterApp::needUpdate(const GitHubRelease& release)
{
    auto current_ver = Version(QString("%1.%2.%3").arg(m_nexoraVersionMajor).arg(m_nexoraVersionMinor).arg(m_nexoraVersionPatch));
    return current_ver < release.version;
}

void NexoraUpdaterApp::downloadError(QString reason)
{
